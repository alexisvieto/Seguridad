...
const modules = [
  ...
  },
  {
    ...
    eyebrow: 'Realtime',
    headline: 'Nunca se pierda una novedad de campo.',
    body: 'Banner de alerta instantáneo cuando un agente reporta una novedad desde campo. Aparece en cualquier página del dashboard sin necesidad de estar en el monitor. El supervisor puede inspeccionar y justificar la incidencia sin cambiar de pantalla.',
    features: ['Supabase Realtime', 'Banner global persistente', 'Modal de inspección', 'Justificación del supervisor', 'Filtrado por rol'],
  },
  {
    id: 'reportes',
    ...
    eyebrow: 'Email + PDF',
    headline: 'El reporte llega antes que el café.',
    body: 'Reporte diario a las 7:45 AM con resumen de novedades vía email. Reporte de cambio de turno en PDF con logo, narrativa contextual y formato profesional. El operador lo completa y lo envía cuando está listo.',
    features: ['Email diario automático', 'PDF con branding', 'Narrativa por puesto', 'Envío manual a gerencia', 'Historial de reportes'],
  },
];
          ...
          <div className="mx-auto max-w-[1200px] px-6 py-16 md:py-20">
            <div className={`grid gap-10 md:gap-16 items-center ${i % 2 === 0 ? 'md:grid-cols-[1fr_380px]' : 'md:grid-cols-[380px_1fr]'}`}>
              ...
              <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.15 }} className={i % 2 !== 0 ? 'md:order-1' : ''}>
                <div className="rounded-xl p-8 flex flex-col items-center justify-center min-h-[280px]" style={{ background: 'var(--ng-bg-float)', border: '1px solid var(--ng-border)' }}>
                  <div className="flex h-16 w-16 items-center justify-center rounded-2xl mb-4" style={{ background: 'var(--ng-lime-bg)', border: '1px solid var(--ng-lime-border)' }}>
                    <span style={{ color: 'var(--ng-lime)' }}>{mod.icon}</span>
                  </div>
                  <p style={{ fontFamily: 'var(--ng-font-display)', fontWeight: 700, fontSize: '1.2rem' }}>{mod.name}</p>
                  <span className="mt-2 rounded-full px-3 py-0.5" style={{ fontFamily: 'var(--ng-font-body)', fontSize: '0.7rem', fontWeight: 600, color: 'var(--ng-lime)', background: 'var(--ng-lime-bg)', border: '1px solid var(--ng-lime-border)' }}>
                    {mod.eyebrow}
                  </span>
                </div>
              </motion.div>
